...
-- The loops intentionally drop any current FK on the target column first, regardless of its old name.
do $$
declare
...
begin
    ...
    loop
        execute format('alter table public.activities drop constraint %I', v_constraint_name);
    end loop;
...
begin
    for v_constraint_name in
        ...
        where tc.constraint_type = 'FOREIGN KEY'
          ...
          and kcu.column_name = 'user_id'
    loop
        execute format('alter table public.user_achievements drop constraint %I', v_constraint_name);
    end loop;
...
begin
    ...
    loop
        execute format('alter table public.referrals drop constraint %I', v_constraint_name);
    end loop;
